...
import TapMouse from "./mouse";
import Tap from "./tap";
import TapTouch from "./touch";
 ...
 */
class Taps {
  _element: Window & typeof globalThis;
  _dragDistance: number;
  _dragDelay: number;
  _index: Map<number, Tap>;
  private _mouse: TapMouse;
  private _touch: TapTouch;
  ...
  *_iterator(property: keyof Tap) {
    for (const [, tap] of this._index) {
      if (tap[property]) yield tap;
    }
  }
